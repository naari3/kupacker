...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data = read_file(args[1].to_string());
...
fn element(input: &[u8]) -> IResult<&[u8], Element> {
    map_res(
        ...
        |strs: Vec<&[u8]>| -> Result<Element, std::num::ParseIntError> {
            if strs.len() == 1 {
                return Ok(Element::Number(std::str::from_utf8(strs[0]).unwrap().parse()?));
            } else {
...
#[test]
fn element_test() {
    assert_eq!(element("(1234)".as_bytes()), Ok(("".as_bytes(), Element::Number(1234))));
    assert_eq!(
